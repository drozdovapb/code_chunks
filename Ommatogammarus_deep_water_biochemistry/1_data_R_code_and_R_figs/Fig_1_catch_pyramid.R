...
datadepth[datadepth$Species == "O. albinus", "numberwithspecies"] <- 
  datadepth[datadepth$Species == "O. albinus", "Approx..number.of.animals"] * (-1)
...
ggplot(datadepth, aes(y = numberwithspecies, x = `Depth,.m`, fill=Species, col=Species)) + 
  ...
  theme(strip.text.x = element_text(face = "italic", size = 12), 
         panel.grid.minor.y=element_blank()) #legend.position="bottomleft",
